...
func TestSmartChequeRepository_GetSmartChequeAnalyticsByPayee(t *testing.T) {
	...
	mock.ExpectQuery("SELECT status, COUNT\\(\\*\\) FROM smart_checks WHERE payee_id = \\$1 GROUP BY status").
		WithArgs("payee1").
	...
	mock.ExpectQuery("SELECT currency, COUNT\\(\\*\\) FROM smart_checks WHERE payee_id = \\$1 GROUP BY currency").
		WithArgs("payee1").
		WillReturnRows(sqlmock.NewRows([]string{"currency", "count"}).
			AddRow("USDT", 6).
			AddRow("USDC", 4))

	mock.ExpectQuery("SELECT COALESCE\\(SUM\\(amount\\), 0\\) as total_amount, COALESCE\\(AVG\\(amount\\), 0\\) as average_amount, COALESCE\\(MAX\\(amount\\), 0\\) as largest_amount, COALESCE\\(MIN\\(amount\\), 0\\) as smallest_amount FROM smart_checks WHERE payee_id = \\$1").
		WithArgs("payee1").
	...
	mock.ExpectQuery("SELECT id, payer_id, payee_id, amount, currency, milestones, escrow_address, status, contract_hash, created_at, updated_at FROM smart_checks WHERE payee_id = \\$1 ORDER BY created_at DESC LIMIT \\$2 OFFSET \\$3").
	...
	mock.ExpectQuery("SELECT DATE\\(created_at\\) as creation_date, COUNT\\(\\*\\) as count FROM smart_checks WHERE payee_id = \\$1 AND created_at >= CURRENT_DATE - INTERVAL '30 days' GROUP BY DATE\\(created_at\\) ORDER BY creation_date").
		WithArgs("payee1").
...
func TestSmartChequeRepository_BatchGetSmartCheques(t *testing.T) {
	...
	checks, err := repo.BatchGetSmartCheques(context.Background(), []string{})
	require.NoError(t, err)
	...
	checks, err = repo.BatchGetSmartCheques(context.Background(), []string{"id1", "id2"})
	require.NoError(t, err)
	assert.NotNil(t, checks)
	assert.Len(t, checks, 2)
	assert.Equal(t, "id1", checks[0].ID)
	assert.Equal(t, "id2", checks[1].ID)
}
